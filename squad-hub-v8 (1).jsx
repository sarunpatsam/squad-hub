    ...
    <div style={{background:isMe?`linear-gradient(135deg,${C.bg2},rgba(16,185,129,0.08))`:C.bg2,border:`1.5px solid ${isMe?C.borderHi:C.border}`,borderRadius:16,padding:"13px 14px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:900,color:isMe?C.greenBr:C.text}}>{player.name}{isMe&&<span style={{fontSize:10,color:C.green}}> · คุณ</span>}</div>
...
export default function SquadHub() {
  ...
  const finishRegister = () => {
    ...
    setPlayer({
      name:regData.nickname, id:`SQ-${Math.floor(Math.random()*900)+100}`,
      position:regData.position, playstyle:regData.playstyle,
      nick:ni?.nick||"", tags:ni?.tags||[], tier:"Bronze",
      reliability:100, level:1, xp:5, stats, ovr:OVR(stats),
  ...
  const doJoin = (teamId) => {
    ...
    setTeams(prev=>prev.map(t=>t.id===teamId?{...t,players:[...t.players,me]}:t));
    setMyTeam(teamId);
  ...
  const renderHome = () => (
    <div style={{paddingTop:16}}>
      ...
      <div onClick={()=>{setVenue(VENUES[0]);setSlot(VENUES[0].slots[1]);setTeams(SEED_TEAMS());setMyTeam(null);setLobbyTab("pitch");setTab("room");}}
